...
with tab1:
    ...
    Units = np.round(new_data['Units Sold'].sum() / 1_000, 2)
...
# --- Summary Tab ---
with tab2:
    d2 = new_data.groupby('Discount Band')['Discounts'].sum().reset_index()
    fig3 = px.pie(d2, names='Discount Band', values='Discounts', hole=0.6,
    ...
    d4 = new_data.groupby(['Country', 'Product'])['Units Sold'].sum().reset_index()
    fig5 = px.bar(d4, x='Country', y='Units Sold', color='Product', barmode='group',
